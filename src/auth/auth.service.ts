...
export class AuthService {
  ...
  async getSignedDoc(signer: string, nonce: string): Promise<string> {
    const appHostName = this.configService.get('providerConsoleAppHost');
  ...
  async verifySignature(
    ...
    if (!verify) {
      throw new ApplicationException('N5005', 'Signature verification failed.');
    }
  ...
  async verifyRefreshToken(
    ...
    if (currentTime > payload['exp']) {
      throw new ApplicationException('N5010', 'Refresh token has expired!');
    }
    ...
    if (refreshTokenDetail == false || refreshTokenDetail == undefined) {
      throw new ApplicationException('N4041', 'Refresh token does not exist');
    }
    // Check refresh token is valid
    if (refreshTokenDetail.valid == false) {
      throw new ApplicationException('N5011', 'Refresh token is not valid');
    }
    ...
    if (currentRefreshToken !== refreshToken) {
      ...
      throw new ApplicationException('N5013', 'Refresh token does not match');
    }
